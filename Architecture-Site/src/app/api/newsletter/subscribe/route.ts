...
async function notifyAdmins(subscriber: NewsletterSubscriber, source: string) {
  ...
  const rows: Prisma.NotificationCreateManyInput[] = admins.map((admin) => ({
    ...
    message: `${subscriber.email} subscribed from Ractysh Architecture.`,
    project: division,
    division,
    priority: "medium",
    entity: "NewsletterSubscriber",
    entityId: subscriber.id,
    actionUrl: "/architecture/dashboard",
    metadata: {
      source,
      submittedFrom: "ractysh-architecture-site",
    },
    ...
    if (created) {
      ...
      after(() =>
        sendArchitectureNewsletterWelcomeEmail({
          ...
          source,
          websiteUrl,
          idempotencyKey: `architecture-newsletter-welcome-${subscriber.id}`,
        })
          .then((delivery) => {
            if (delivery.sent) {
