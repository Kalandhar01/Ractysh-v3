...
float pattern(vec2 p) {
  vec2 drift = vec2(-time * waveSpeed * 0.72, -time * waveSpeed * 0.42);
  vec2 shimmer = vec2(
    sin((p.y + time * waveSpeed * 0.5) * 6.0),
    cos((p.x - time * waveSpeed * 0.45) * 4.5)
  ) * 0.055;
  return fbm(p + shimmer + fbm(p + drift)); 
}
...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    const uniforms =
      (shaderMaterial.current?.uniforms as WaveUniforms | undefined) ??
    ...
    if (uniforms.waveSpeed.value !== waveSpeed) {
