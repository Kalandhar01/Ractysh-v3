...
export const BackgroundBeams = React.memo(
  ({ className, pathStep = 3, strokeOpacity = 0.42 }: BackgroundBeamsProps) => {
    const safePathStep = Math.max(1, Math.floor(pathStep));
    const animatedPaths = paths
      .map((path, index) => ({ index, path }))
      .filter(({ index }) => index % safePathStep === 0);
        ...
        >
          ...
          <defs>
            {animatedPaths.map(({ index }) => (
              ...
              >
                <stop stopColor="#16b893" stopOpacity="0" />
                <stop stopColor="#16b893" />
                <stop offset="32.5%" stopColor="#f3c987" />
                <stop offset="100%" stopColor="#bd791d" stopOpacity="0" />
              </motion.linearGradient>
            ))}
